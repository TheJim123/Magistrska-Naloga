...
\begin{lema}
	\label{lem:invnorm}
	Naj bosta $X$ ter $Y$ poljubna normirana prostora in naj bo $T\in B(X, Y)$ poljuben omejen linearen operator. Če je $T$ obrnljiv, potem za vse $x\in X$ velja: $$\norm{Tx}\geq \norm{T^{-1}}^{-1}\norm{x}.$$
\end{lema}
...
\begin{izrek}
	\label{izr:Adjexist}
	Naj bosta $(X, \Sp{.}{.}_X)$ in $(Y, \Sp{.}{.}_Y)$ poljubna kompleksna Hilbertova prostora in naj bo $T$ poljuben element $B(X, Y)$. Tedaj obstaja enolično določen operator $T^*\in B(Y, X)$, tak, da velja $$\Sp{Tx}{y}_Y = \Sp{x}{T^*y}_X$$ za vse $x\in X$ in vse $y \in Y$.
\end{izrek}

\begin{dokaz}
	Naj bo $y\in Y$ poljuben element ter naj bo $\map{f_y}{X}{\C}$ preslikava definirana s predpisom $f_y(x) = \Sp{Tx}{y}_Y$. Opazimo, da je $f_y$ linearen funkcional na $X$. Po neenakosti Cauchy-Schwarz-Bunyakowsky potem za vsak element $x\in X$ velja: $$\abs{f_y(x)} = \abs{\Sp{Tx}{y}_Y} \leq \norm{Tx}_Y \cdot \norm{y}_Y \leq \norm{T}\cdot\norm{x}_X\cdot\norm{y}_Y.$$ To pomeni, da je $f_y$ omejen linearen funkcional na $X$. Posledično po Riezsovem izreku obstaja tak enolično določen $z\in X$, da je $f_y(x) = \Sp{x}{z}_X$ za vse $x\in X$. Sedaj definiramo preslikavo $\map{T^*}{Y}{X}$, ki vsakemu $y\in Y$ priredi pripadajoči $z\in X$ (tisti, enolično določen, za katerega je $f_y(x) = \Sp{x}{z}_X$ za vse $x\in X$). Posledično za vsak $x\in X$ in vsak $y\in Y$ velja: $$\Sp{Tx}{y}_Y=\Sp{x}{T^*y}_X .$$
	...
	\end{align*}
	To pomeni, da je $T^*(\alpha y_1 + \beta y_2)=\alpha T^*y_1 + \beta T^*y_2$ za vse $x\in X$, vse $y_1, y_2 \in Y$ in vse $\alpha, \beta \in \C$, torej je $T^*$ res linearna preslikava. S pomočjo neenakosti Cauchy-Schwarz-Bunyakowsky vidimo tudi, da velja $$\norm{T^*y}_X^2 = \Sp{T^*y}{T^*y}_X = \Sp{TT^*y}{y}_Y \leq \norm{TT^*y}_Y\cdot\norm{y}_Y \leq \norm{T}\cdot\norm{T^*y}_X\cdot\norm{y}_Y$$ za vse $y\in Y$.
	...
	Preslikava $T^*$ je torej omejen linearen operator in velja, da je $\norm{T^*}\leq \norm{T}$. Za konec še pokažimo, da je $T^*$ enolično določen. Denimo, da imamo $U_1, U_2 \in B(Y, X)$, za katera velja, da je $\Sp{Tx}{y}_Y = \Sp{x}{U_1 y}_X = \Sp{x}{U_2 y}_X$ za vse $x\in X$ in vse $y\in Y$. Tedaj velja, da je $U_1y = U_2y$ za vse $y\in Y$, torej je $U_1 = U_2$. To pomeni, da je $T^*$ res enolično določen.
\end{dokaz}
...
\begin{primer}
	...
	Opazimo, da če velja $z_i = y_{i+1}$ za vse $i\in \N$, potem bo naša enačba veljavna za poljubno zaporedje $x$. Enoličnost adjungiranega operatorja nam posledično da predpis $$D^*y = D^*(y_1, y_2, y_3 \ldots) = (y_2, y_3, y_4, \ldots)$$ za vse $y\in l^2$. Še več, prepoznamo, da je dobljeni operator ravno t.~i.~ ">levi premik"<, ki ga tipično označimo z $L$.
\end{primer}
...
\begin{lema}
	...
	Naj bosta $(X, \Sp{.}{.}_X)$ in $(Y, \Sp{.}{.}_Y)$ poljubna kompleksna Hilbertova prostora. Tedaj za vsak operator $T\in B(X, Y)$ velja: \begin{enumerate}[a)]
		...
		\item $\operatorname{Ker}T^* = (\operatorname{Im}T)^{\bot}$,
		\item $\operatorname{Ker}^* = \{0\} \iff \operatorname{Im}T$ je gosta množica v $Y$.
	\end{enumerate}
\end{lema}
...
\begin{primer}
	\label{prim:normalop}
	Enostavno je videti, da je operator $T_k$, kot je bil definiran v primeru \ref{prim:adjop2}, normalen operator. Naj bo $f\in \mathcal{L}^2([0, 1])$ poljuben. Velja:
	\begin{align*}
...
\begin{lema}
	...
	Naj bo $(X, \Sp{.}{.})$ poljuben kompleksen Hilbertov prostor. Naj bo $T\in B(X)$ poljuben normalen operator ter $r > 0$ poljuben. Velja: \begin{enumerate}[a)]
		...
		\item če je $\norm{Tx} \geq r\norm{x};~\forall x\in X$, je $\operatorname{Ker}T^* = \{0\}$.
	\end{enumerate}
\end{lema}
...
\begin{dokaz}
	\begin{enumerate}[a)]
		...
		\item Denimo, da obstaja tak $r>0$, da je $\norm{Tx} \geq r\norm{x};~\forall x\in X$. Naj bo $x\in \operatorname{Ker}T^*$ poljuben. Po točki $a)$ tega dokaza je potem $0 = \norm{T^*x} = \norm{Tx} \geq r\norm{x}$. Sledi, da je $\norm{x} = 0$ oz. $x = 0$. Posledično je $\operatorname{Ker}T^* = \{0\}$.
	\end{enumerate}
\end{dokaz}
...
\begin{dokaz}
	...
	\begin{itemize}
		...
		\item[$\Leftarrow)$] Denimo, da obstaja tak $r>0$, da je $\norm{Tx} \geq r\norm{x}~\forall x\in X$. Po lemi \ref{lem:normalnorm} je potem $\operatorname{Ker}T^* = \{0\}$. Po posledici \ref{posl:adjinv} je potem $T$ obrnljiv.
	\end{itemize}
\end{dokaz}
...
\begin{izrek}
	...
	Naj bo $(X, \Sp{.}{.})$ poljuben kompleksen Hilbertov prostor. Naj bosta $T, U\in B(X)$ poljubna. Velja: \begin{enumerate}[a)]
		...
		\item $U$ je unitaren operator $\iff U$~je surjektivna izometrija na $X$.
	\end{enumerate}
\end{izrek}
...
\begin{dokaz}
	...
	\begin{enumerate}[a)]
		...
		\item Pri dokazu te ekvivalence, si bomo pomagali s prejšnjo točko dokaza.  \begin{itemize}
			\item[$\Rightarrow)$] Denimo, da je $U$ unitaren. Potem sledi, da je $UU^*=U^*U = I$ in po točki $a)$ je potem $U$ izometrija. Dodatno, naj bo $x\in X$ poljuben. Potem je $x = U(U^*x)$, torej je $x\in \operatorname{Im}U$. Sledi, da je $\operatorname{Im}U = X$ oz. $U$ je surjektiven.
